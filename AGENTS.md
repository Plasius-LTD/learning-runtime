...
## 16. Runtime package guidance

This is @plasius/learning-runtime. Source lives in src/ and tests in tests/. Use Node 24, npm, strict TypeScript and the copied schema tooling conventions.

Only bounded execution and deterministic simulation belong here. No account storage, HTTP, authentication, payment, physical hardware, network or UI access. Hosts retain protected assessment cases and sole authority for progress and completion. Learner source and structured outputs are untrusted. Enforce source/input/output/action/depth/memory/time budgets and dispose sessions on cancellation or failure. Never publish protected host scenarios, learner records, tokens or source.
